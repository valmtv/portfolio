    ...
    <div className="flex gap-5 md:gap-8">
      ...
      <div className={cn("flex-1 min-w-0", !isLast && "pb-12 md:pb-16")}>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-2">
          ...
          <span className="text-theme-border text-sm select-none">·</span>
          <span className={cn("text-sm text-theme-mutedForeground", classes.body)}>{project.type}</span>
        </div>
        ...
        <ul className="space-y-2 mb-5">
          {project.bullets.map((bullet, i) => (
            <li key={i} className={cn("flex gap-2 text-sm md:text-base text-theme-mutedForeground", classes.body)}>
              <span className="text-theme-primary flex-shrink-0 mt-0.5" aria-hidden>
                →
              </span>
              <span>{bullet}</span>
            </li>
        ...
        <div className="flex flex-wrap gap-2">
          {project.techStack.map((tech) => (
            <span
              key={tech}
              className={cn(
                "px-2 py-1 text-xs font-medium bg-theme-accent text-theme-accentForeground border border-theme-border",
                classes.body
              )}
            >
              {tech}
